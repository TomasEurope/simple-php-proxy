...
$app->any('/{path:.*}', function (Request $request, Response $response) {
    ...
    try {
        ...
        d('Server Request', $guzzleRequest);
        ...
        if (!DEBUG) {
            ...
            d('Response size', strlen($contents), $contents !== '' ? 'green' : 'red');
            d('Response preview', substr(htmlentities($contents), 0, 1000), $contents !== '' ? 'green' : 'red');
        }
    } catch (Exception $e) {
        // Handle exceptions during the proxy process
        $response = $response->withStatus(599);
        d('Error', $e->getMessage(), 'red', true);
        d('Error body', $response, 'red', true);
    }
